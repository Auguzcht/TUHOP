// @ts-nocheck
/* eslint-disable @typescript-eslint/no-explicit-any, react-hooks/set-state-in-effect, react-hooks/exhaustive-deps, react-hooks/refs, react-hooks/immutability, react-refresh/only-export-components */
...
function MapLocatePulseIcon() {
    return (
        <div style={{position: "relative", width: 24, height: 24}}>
            <div style={{position: "absolute", inset: -4, borderRadius: "50%", background: "oklch(0.65 0.14 185)", animation: "ping 1.5s cubic-bezier(0,0,0.2,1) infinite", opacity: 0.75}} />
            <div style={{position: "relative", width: 24, height: 24, borderRadius: "50%", background: "oklch(0.65 0.14 185)", border: "2px solid white", boxShadow: "0 2px 6px rgba(0,0,0,0.3)"}} />
        </div>
    )
}
...
/**
 * Renders barangay boundary polygons from Supabase GeoJSON.
 * Handles fetching via get_barangay_boundaries RPC.
 * Must be used inside MapLayers context.
 */
function MapBoundaryLayer({
    features: propFeatures,
    colorMap,
    highlightId,
}: {
    /** Pre-fetched features from React Query (optional — skips internal fetch) */
    features?: Record<string, unknown>[] | null
    /** Return a fill color for a barangay by name, or null for default */
    colorMap?: (barangayName: string) => string | null
    /** Highlight this barangay ID with a teal border and brighter fill */
    highlightId?: string
}) {
    const { L } = useLeaflet()
    const [localFeatures, setLocalFeatures] = useState<Record<string, unknown>[] | null>(null)
    const features = propFeatures ?? localFeatures

    // Internal fetch fallback (only if no features prop provided)
    useEffect(() => {
        if (propFeatures !== undefined) return
        if (!L) return
        const url = `${import.meta.env.VITE_SUPABASE_URL}/rest/v1/rpc/get_barangay_boundaries`
        ...
        fetch(url, {
            ...
            .then((data: Record<string, unknown>) => {
                const fc = data as { features?: Record<string, unknown>[] }
                setLocalFeatures(fc.features ?? null)
            })
            .catch(() => {})
    }, [L, propFeatures])
    ...
    const defaultFill = "rgba(100,116,139,0.08)"

    return (
        <>
            {features.map((f, i) => {
                const props = f.properties as Record<string, unknown> ?? {}
                const name = ((props.descrptn as string) ?? "").trim().toUpperCase()
                const geom = f.geometry as Record<string, unknown> | null
                if (!geom || !geom.coordinates) return null

                const isHighlighted = highlightId && (props?.id as string) === highlightId
                const fillColor = isHighlighted ? "rgba(44,181,160,0.15)" : (colorMap?.(name) ?? defaultFill)
                const coords = geom.coordinates as number[][][][]
                const polys = geom.type === "MultiPolygon" ? coords : [coords]

                return polys.map((ring, ri) => {
                    const positions = (ring[0] as [number, number][]).map((p: number[]) => [p[1], p[0]] as [number, number])
                    return (
                        <LeafletPolygon
                            key={`b-${i}-${ri}`}
                            positions={positions}
                            pathOptions={{
                                ...
                                color: isHighlighted ? "oklch(0.65 0.14 185)" : "rgba(100,116,139,0.5)",
                                fillOpacity: 1,
                            }}
                        />
                    )
